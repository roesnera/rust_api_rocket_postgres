...
fn test_view_rustacean() {
    ...
    let saved_rustacean: Value = client.get(format!("{}/rustaceans/{}", common::APP_HOST, rustacean["id"])).send().unwrap().json().unwrap();
...
fn test_delete_rustacean() {
    ...
    let response = client.delete(format!("{}/rustaceans/{}", common::APP_HOST, rustacean["id"])).send().unwrap();
